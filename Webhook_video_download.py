...
HTTP_STATUS_CODE = { 
    ...
    502: 'API had a problem (Bad Gateway)',
    503: 'API had a problem (503)',
    504: 'API had a problem (Gateway timeout)'
    }
...
###
# Step 5: Prefect the videos in FLV format, returns immediately with a UUID. 
# The success_hook is called when the video is uploaded successfully.  
# The failure_hook is called if there was a problem retrieving the video..
###

for curr_video in video_list:
    url = "https://%s.eagleeyenetworks.com/asset/cloud/video.flv" % current_user['active_brand_subdomain']

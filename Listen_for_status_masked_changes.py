...
for d in camera_id_list:
    register_msg['cameras'][d] = { 'resource': ['status_masked'] }
...
while True:
    ...
    for device_id, device_data in jdata['data'].items():
        ret = parseStatusDecimal(device_data['status_masked'])
        print(CLI_STATUS_STRING.format(device_id, hex(device_data['status_masked']), ret))

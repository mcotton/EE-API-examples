...
async def follow():
    #uri = "ws://%s.a.plumv.com:28080/api/v2/ws_poll2"%accountid
    uri = 'wss://login.eagleeyenetworks.com/api/v2/Device/{}/Events?A={}'.format(account_id, auth_key)
    print(uri)
    async with websockets.connect(uri) as ws:
        ...
        async for msg in ws:
            jdata = json.loads(msg) #convert the json string to a python dictionary/array
...
while(1):
    try:
        asyncio.get_event_loop().run_until_complete(follow())
    except KeyboardInterrupt:
        raise
    except:
        raise
        print(datetime.datetime.utcnow(),"websocket closed, trying again")
